...
/*   Updated: 2022/09/19 22:12:02 by tburakow         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_heat_axial(t_heat *heat, t_coords *cell, int value)
{
	...
	ret = 0;
	if (cell->y > 0)
		ret += check_up(heat, cell, value);
	if (cell->x > 0)
		ret += check_left(heat, cell, value);
	if (cell->y + 1 < heat->size.h)
		ret += check_down(heat, cell, value);
	if (cell->x + 1 < heat->size.w)
		ret += check_right(heat, cell, value);
	return (ret);
}
...
int	check_heat_diag(t_heat *heat, t_coords *cell, int value)
{
	...
	if (cell->y > 0 && cell->x > 0)
		ret += check_up_left(heat, cell, value);
	if (cell->x > 0 && cell->y + 1 < heat->size.h)
		ret += check_down_left(heat, cell, value);
	if (cell->y + 1 < heat->size.h && cell->x + 1 < heat->size.w)
		ret += check_down_right(heat, cell, value);
	if (cell->x + 1 < heat->size.w && cell->y > 0)
		ret += check_up_right(heat, cell, value);
	return (ret);
}
